          ...
          <div className="tour__header">
            ...
            <div className="tour__details">
              <div className="save">
                {currentUser ? (
                  <button
                    onClick={controlBookmark}
                    className={`btn cardLarge__save icon icon__save bookmarkIcon_${tour._id}`}
                  >
                    {bookmarks.length === 0 ? (
                      <div
                        className={`icon_${tour._id} cardLarge__save icon icon__save`}
                      >
                        <ion-icon
                          size="large"
              ...
              <div className="tour__detailsLinks">
                <Link to="/login" className="btnAuthSignUp tour__detailsLink">
              ...
              <h4 className="heading heading__4 tour__detailsTitle">
                {tour.name}
              </h4>
              <div className="tour__detailsDays">
                ...
                <div className="tour__detailsStar">
                  ...
                  <div className="text__description text__description--dark">
                    <Link to={`${tour._id}`} className="link">
                      {tour.ratingQuantity}
                    </Link>
                  </div>
                </div>
              </div>
              <p className="text__description text__description--dark tour__detailsStarts">
                ...
                {tour.keywords.map((word) => {
                  return (
                    <Link
                      to={`${tour._id}`}
                      key={`keywords.${word}`}
                      className="tour__detailsKeyword"
                    >
                      {word}
                    </Link>
                  );
              ...
              <div className="tour__detailsPrice">
                <p className="tour__detailsEuro">
                  €{tour.price}
                  <span className="text__description text__description--dark">
                    <br />
                    /per person
                  </span>
                </p>
                {shoppingCartFiltered.find((el) => el._id === tour._id) ? (
                  <button
                    onClick={controlShoppingCart}
                    className="tour__detailsBtn btn btn__main btn__inactive"
                  >
                    in cart
                  </button>
                ) : (
                  <Link
                    to="/shoppingCart"
                    onClick={controlShoppingCart}
                    className="tour__detailsBtn btn btn__main"
                  >
                    book now
                  </Link>
                )}
          ...
          <div className="tour__description">
            ...
            <div className="tour__checklist">
              {tour.descriptionCheckPoints.map((point) => {
                return (
                  <div
                    key={`checkpoints.${point}`}
                    className="tour__checklist--flex"
                  >
